...
module.exports = function(source, callback) {
	...
	var on = outcome.error(callback), loadingCache = false;
	...
	if(source.indexOf('http://') == 0) {
		...
		//on response, check for cache
			...
			cache.get(source, lastModified, function(hasCache, file) {
				if(!file) return;
	...
	//fs?
		...
		fs.stat(source, on.success(function(stat) {
			...
			//get the cache
			cache.get(source, stat.mtime, function(hasCache, file) {
	...
	function onLoad(lastModified, content) {
		...
		wrap(content, on.success(function(wrappedContent) {

			//cache the wrapped content so the JS isn't processed again
			cache.set(source, lastModified, wrappedContent);
